...
sort -k1,1 ./scripts/num_seqs.csv > dpa_seqs_sorted.csv
...
tr , '\n' < dpa_mem_to_transpose.csv > $2/dpaVMEM.$1.csv
sed 's/^ *//' $2/dpaVMEM.$1.csv
...
sort -k1,1 ./scripts/num_seqs.csv > std_seqs_sorted.csv
...
tr , '\n' < std_mem_to_transpose.csv > $2/stdVMEM.$1.csv
sed 's/^ *//' $2/stdVMEM.$1.csv
...
sort -k1,1 ./scripts/num_seqs.csv > dft_seqs_sorted.csv
...
tr , '\n' < dft_mem_to_transpose.csv > $2/dftVMEM.$1.csv
sed 's/^ *//' $2/dftVMEM.$1.csv
...
sort -k1,1 ./scripts/num_seqs.csv > tree_seqs_sorted.csv
...
echo $(IFS=$'\n'; 
    for i in `cat tree_mem_sorted.csv | cut -f 1,2,4 -d " " | sort -u | sort -k4,4n`;
    do
        echo $(echo $i; grep $(echo "$i ") tree_mem_sorted.csv | cut -f1 -d " "; echo ",") ; 
    done) > tree_mem_to_transpose.csv
tr , '\n' < tree_mem_to_transpose.csv > $2/treeVMEM.$1.csv
sed 's/^ *//' $2/treeVMEM.$1.csv
