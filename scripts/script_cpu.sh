...
grep 'COMPLETED\|CACHED' | cut -f 4,5,7 | awk -F "." '{print $1,$2,$3,$4,$5,$6}' | \
...
sort -k5,5n -k2,2  tree_cpu_and_numseqs.csv > tree_cpu_sorted.csv

echo $(IFS=$'\n'; 
    for i in `cat tree_cpu_sorted.csv | cut -f 1,2,4 -d " " | sort -u | sort -k4,4n`;
    do
        echo $(echo $i; grep $(echo "$i ") tree_cpu_sorted.csv | cut -f1 -d " "; echo ",") ; 
